package macortesn.swenterprisemanagersystem.Model;
...
public class Enterprise {
    ...
    public String toString() {
        ...
        String[] allClassif = classification.split(";");
        String stringClassif ="";
        for(int i =0; i<allClassif.length;i++)
            stringClassif+=allClassif[i] + " | ";


        String[] allProducts = products.split(" ");
        String stringProducts = "";
        for(int j=0; j<allProducts.length;j++)
            stringProducts+="     - " + allProducts[j] + "\n";
        return "Empresa " +
                id + "\n" +
                "  Nombre : " + name + "\n" +
                "  Url : " + url + "\n" +
